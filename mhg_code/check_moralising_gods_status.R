...
# exists in 41586_2019_1043_MOESM6_ESM.xlsx to ensure that the code in
# ./mhg_code gives identical results. This is done for both MoralisingGods and
# DoctrinalMode. The check fails for DoctrinalMode for two NGAs, Middle Yellow
# River Valley and Orkhon Valley. I am uncertain what causes the discrepancy.
# However, only the MoralisingGods data are used in our article, The Past as a
# Stochastic Process.

rm(list=ls()) # Clear the workspace
natureData <- read.csv('./other_csv/41586_2019_1043_MOESM6_ESM_sheet1.csv',stringsAsFactors=F)
githubData <- read.csv('data_used_for_nature_analysis.csv',stringsAsFactors=F)
